...
class MedicareRiskOverview:
    ...
    def makedir(self, customer):
        path1 = str(customer) + "/" + str(customer) + "-MedicareRiskOverview"
                    ...
                    try:
                        ...
                        with self.conn:
                            self.cur.execute(
                                ...
                                (int(customer), "MEdicare Risk Overview", int(x), str("Does not exists"),
                                 str("Does not exists"),))
                    ...
                    if self.check_exists_byclass("nodata"):
                        ...
                        try:
                            with self.conn:
                                self.cur.execute(
                                    'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                    (int(customer), "Medicare RiskOverview", int(x), str("Overview"), str(val),))
                                # print 'Data saved from exception'
                                ss_name = str(wbpath) + "/" + str(i) + str(
                                    val) +"Medicare-Risk Overview"+ "Overview" + ".png"  # naming convention is year-lob-drill.png
                            ...
                            if self.check_exists_byclass("nodata"):
                                ...
                                try:
                                    with self.conn:
                                        self.cur.execute(
                                            'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                            (int(customer), "Medicare-Risk Overview", int(x), str(drill_name), str(val)))
                                        # print 'Data saved from exception'
                                        ...
                                        try:
                                            with self.conn:
                                                self.cur.execute(
                                                    'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                                    (
                                                        int(customer), "Medicare-Risk Overview", int(x), str(drill_name2),
                                                        str(val)))
                                                # print 'Data saved from exception'
                                                ss_name = str(wbpath) + "/" + str(i) + str(val) + str(
                ...
                if self.check_exists_byclass("nodata"):
                    ...
                    try:
                        with self.conn:
                            self.cur.execute(
                                'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                (int(customer), "Medicare RiskOverview", int(x), str("Overview"), "Medicare"))
                            # print 'Data saved from exception'
                            ss_name = str(wbpath) + "/" + str(
                                i) + "Medicare" + "Overview" + ".png"  # naming convention is year-lob-drill.png
                            self.driver.save_screenshot(ss_name)
                    except sqlite3.IntegrityError:
                        pass
                            ...
                            try:
                                with self.conn:
                                    self.cur.execute(
                                        ...
                                        'LOB) VALUES (?,?,?,?,?)',
                                        (int(customer), "Medicare RiskOverview", int(x), str(drill_name), "Medicare"))
                                    # print 'Data saved from exception'
                                    ...
                                    try:
                                        with self.conn:
                                            self.cur.execute(
                                                'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                                (
                                                    int(customer), "Quality Overview", int(x), str(drill_name2),
                                                    "Medicare"))
                                            # print 'Data saved from exception'
                                            ss_name = str(wbpath) + "/" + str(i) + "Medicare" + str(
                                                drill_name2) + ".png"  # naming convention is year-lob-drill.png
