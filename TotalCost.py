...
def wait_to_load(driver):
    loader=config.get("TotalCost-Prod","loader_element")
...
class TotalCost:
    ...
    def __init__(self, driver):
        ...
        self.driver = driver
        self.loader_element = config.get("TotalCost-Prod","loader_element")
        self.selected_value_year_xpath = config.get("TotalCost-Prod","selected_value_year_xpath")
        self.service_year_xpath = config.get("TotalCost-Prod","service_year_xpath")
        self.lob_xpath = config.get("TotalCost-Prod","lob_xpath")
        self.lob_elements_xpath = config.get("TotalCost-Prod","lob_elements_xpath")

        self.apply_filter_xpath = config.get("TotalCost-Prod","apply_filter_xpath")

        self.loader_panel_class = config.get("TotalCost-Prod","loader_panel_class")
        self.drilldown_elements_xpath = config.get("TotalCost-Prod","drilldown_elements_xpath")
        self.select_all_id = config.get("TotalCost-Prod","select_all_id")
        self.overview_xpath = config.get("TotalCost-Prod","overview_xpath")
        self.workbook_name= "Total Cost"
    ...
    def makedir(self, customer):
        path1 = str(customer) + "/" + str(customer) + "-Total Cost"
    ...
    def iterate_filter(self, year, customer):
        ...
        for i in year:
                ...
                try:
                    ...
                    with self.conn:
                        self.cur.execute(
                            ...
                            (int(customer), "Utilization-Total Cost", int(i), str("Does not exist"),
                             str("Does not  exists"),))
            ...
            for lob_element in lob_elements:
                ...
                lob_selector_xpath = "//label[@class=\"radio\"]//input[@value=\"%s\"]//following-sibling::span "% (val)
                lob_selector=self.driver.find_element_by_xpath(lob_selector_xpath)
                # print(st)
                self.action_click(lob_selector)
                not_closed = 0
                while (not_closed <= 1):
                    try:
                ...
                if self.check_exists_byclass("nodata"):
                    ...
                    try:
                        with self.conn:
                            self.cur.execute(
                                'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                (int(customer), "Total cost", int(i), str("By Service Category"), str(val),))
                            # print 'Data saved from exception'
                            ss_name = str(wbpath) + "/" + str(i) + str(
                                val) + "Service Category" + ".png"  # naming convention is year-lob-drill.png
