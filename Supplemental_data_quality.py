...
ws.append(["ID", "LoB", "Metric Name/ID", "Provider","Patient ID", "Scenario", "Status", "Comments", "URL"])
...
for i in range(0, len(LOB_list)):
    ...
    for measure_index, measure in enumerate(final_measure_list):
        ...
        if len(registry_element.find_elements_by_id(measure[1])) > 0:
            registry_element.find_element_by_id(measure[1]).click()
            print('validating')
            sf.ajax_preloader_wait(driver)
            ...
            if "No data available" in first_provider_element.text:
                ...
                # time.sleep(1)
                # registry_element = driver.find_element_by_id("registry_body").find_element_by_tag_name("ul")
                ws.append([test_case_id, LOB_Name, measure[0]+"_"+measure[1], "x", "x", "Navigate to MSPL of a provider", "Failed", "No providers in Support level", driver.current_url])
                continue
            else:
                ...
                if "No Data Available" in table[0].text:
                    ws.append(
                        [test_case_id, LOB_Name, measure[0] + "_" + measure[1], provider_name,"x", "Navigate to a patient's Dashboard",
                         "Failed", "No Patients in MSPL", driver.current_url])
                    driver.get(LOB_Specific_URL)
                    sf.ajax_preloader_wait(driver)
                    WebDriverWait(driver, 60).until(
                        EC.presence_of_element_located((By.XPATH, locator.xpath_filter_measure_list)))
                ...
                if len(first_patient_element.find_elements_by_xpath("//td[@class=' pencil_icon pencil_icon_pt']")) > 0:
                    ...
                    if "Add Supplemental Data" in first_patient_element.find_element_by_xpath("//td[@class=' pencil_icon pencil_icon_pt']").get_attribute("innerHTML"):
                        ...
                        for dashboard_measure in non_complaint_measures:
                            ...
                            if "BCS" in dashboard_measure.find_element_by_xpath(".//div[@class='tiny-text text-grey valign-wrapper']").get_attribute("outerHTML"):
                                    ...
                                    if "Add Supplemental Data" in pencil_option.text:
                                        ...
                                        try:
                                            ...
                                            if sf.URLAccessCheck(driver.current_url, driver):
                                                ws.append(
                                                    [test_case_id, LOB_Name, measure[0] + "_" + measure[1],
                                                     ...
                                                     "Failed", "Error Toast message was displayed", driver.current_url])
                                                ...
                                                ws.append(
                                                    [test_case_id, LOB_Name, measure[0] + "_" + measure[1],
                                                     provider_name, patient_id, "Attaching a File",
                                                     "Passed", "", driver.current_url])
                                                supdata = driver.find_element_by_class_name("saved_data")
                                                DoS = datetime.datetime.now()
                                                DoS = DoS.strftime("%m/%d/%Y")
                                                supdata.find_element_by_xpath("//input[@class='materialize_datefilter datepicker no-future-dt form-control form-text required']").\
                                                    send_keys(DoS)
                                                time.sleep(2)
                                                ...
                                                time.sleep(1)
                                                code_box.click()
                                                codes = driver.find_element_by_xpath("//ul[@class='dropdown-content mat-ac-dropdown quality_ac_dropdown']").find_elements_by_tag_name('li')
                                                time.sleep(1)
                                                codes[0].click()
                                                time.sleep(1)
                                                ...
                                                if sf.URLAccessCheck(driver.current_url, driver):
                                                    ws.append(
                                                        [test_case_id, LOB_Name, measure[0] + "_" + measure[1],
                                                         ...
                                                         "Failed", "Error Toast message was displayed",
                                                         driver.current_url])
                                                ...
                                                bcs_measure_xpath = "//div[contains(text(),'BCS · Preventive Health Screening')]//..//..//..//..//..//.."

                                                start_time = time.perf_counter()
                                                end_time = int(start_time)+60
                                                while True:
                                                    try:
                                                        measure_element = driver.find_element_by_xpath(bcs_measure_xpath)
                                                    except Exception as e:
                                                        traceback.print_exc()
                                                        ws.append(
                                                            [test_case_id, LOB_Name, measure[0] + "_" + measure[1],
                                                    ...
                                                    if end_time - time.perf_counter() < 1:
                                                        ws.append(
                                                            [test_case_id, LOB_Name, measure[0] + "_" + measure[1],
                                                             provider_name, patient_id, "Red dot Status",
                                                             "Failed", "Taking too long! skipping and attempting delete", driver.current_url])
                                                        break
                                                ...
                                                try:
                                                    ...
                                                    if sf.URLAccessCheck(driver.current_url, driver):
                                                        ws.append(
                                                            [test_case_id, LOB_Name, measure[0] + "_" + measure[1],
                                                             ...
                                                             "Failed", "Error Toast message was displayed, Manual Intervention needed",
                                                             driver.current_url])
                                                    ...
                                                    ws.append(
                                                        [test_case_id, LOB_Name, measure[0] + "_" + measure[1],
                                                         provider_name, patient_id, "Delete Supp Data",
                                                         "Failed", "Manual Intervention needed", driver.current_url])
                                            ...
                                            ws.append(
                                                [test_case_id, LOB_Name, measure[0] + "_" + measure[1],
                                                 provider_name, patient_id, "Attaching a file",
                                                 "Failed", "Error occured in Coding tool, Manual Intervention might be needed", driver.current_url])
                                        break
                        ...
                        ws.append(
                            [test_case_id, LOB_Name, measure[0] + "_" + measure[1], provider_name, "x",
                             ...
                             "Failed", "No \"Add Supplemental Data\" option", driver.current_url])
                        driver.get(LOB_Specific_URL)
                        sf.ajax_preloader_wait(driver)
                        WebDriverWait(driver, 60).until(
                            EC.presence_of_element_located((By.XPATH, locator.xpath_filter_measure_list)))
                    ...
                    ws.append(
                        [test_case_id, LOB_Name, measure[0] + "_" + measure[1], provider_name, "x",
                         "Navigate to a patient's Dashboard",
                         "Failed", "No Pencil icon", driver.current_url])
                    driver.get(LOB_Specific_URL)
                    sf.ajax_preloader_wait(driver)
                    WebDriverWait(driver, 60).until(
                        EC.presence_of_element_located((By.XPATH, locator.xpath_filter_measure_list)))
