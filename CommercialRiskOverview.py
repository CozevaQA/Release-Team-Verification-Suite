...
class CommercialRiskOverview:
    ...
    def makedir(self, customer):
        path1 = str(customer) + "/" + str(customer) + "-CommercialRiskOverview"
                    ...
                    try:
                        ...
                        with self.conn:
                            self.cur.execute(
                                ...
                                (int(customer), "Commercial RiskOverview", int(i), str("Does not exists"), str("Does not exists"),))
                    ...
                    if self.check_exists_byclass("nodata"):
                        ...
                        try:
                            with self.conn:
                                self.cur.execute(
                                    'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                    (int(customer), "Commercial RiskOverview", int(i), str("Overview"), str(val),))
                                # print 'Data saved from exception'
                                ss_name = str(wbpath) + "/" + str(i) + str(
                                    val) +"Commercial-Risk Overview"+ "Overview" + ".png"  # naming convention is year-lob-drill.png
                            ...
                            if self.check_exists_byclass("nodata"):
                                ...
                                try:
                                    with self.conn:
                                        self.cur.execute(
                                            'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                            (int(customer), "Commercial-Risk Overview", int(i), str(drill_name), str(val),))
                                        # print 'Data saved from exception'
                                        ...
                                        try:
                                            with self.conn:
                                                self.cur.execute(
                                                    'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                                    (
                                                        int(customer), "Commercial-Risk Overview", int(i), str(drill_name2),
                                                        str(val),))
                                                # print 'Data saved from exception'
                                                ss_name = str(wbpath) + "/" + str(i) + str(val) + str(
                ...
                if self.check_exists_byclass("nodata"):
                    ...
                    try:
                        with self.conn:
                            self.cur.execute(
                                'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                (int(customer), "Commercial RiskOverview", int(i), str("Overview"), "Commercial",))
                            # print 'Data saved from exception'
                            ss_name = str(wbpath) + "/" + str(
                                i) + "Commercial" + "Overview" + ".png"  # naming convention is year-lob-drill.png
                            self.driver.save_screenshot(ss_name)
                    except sqlite3.IntegrityError:
                        pass
                    ...
                    for j in range(2, x + 1):
                        ...
                        try:
                            ...
                            ss_name = str(wbpath) + "/" + str(i) + "UnknownError-Commercial" + str(
                                drill_name) + ".png"  # naming convention is year-lob-drill.png
                            self.driver.save_screenshot(ss_name)
                            print("Error Occured in loading")
                            ...
                            try:
                                with self.conn:
                                    self.cur.execute(
                                        ...
                                        'LOB) VALUES (?,?,?,?,?)',
                                        (int(customer), "Commercial RiskOverview", int(i), str(drill_name), "Medicare",))
                                    # print 'Data saved from exception'
