...
class MedicareCodingDiscontinuation:
    ...
    def makedir(self, customer):
        path1 = str(customer) + "/" + str(customer) + "-MedicareCodingDiscontinuation"
                    ...
                    try:
                        ...
                        with self.conn:
                            self.cur.execute(
                                ...
                                (int(customer), "Medicare Coding Discontinuation", int(i), str("Does not exists"),
                                 str("Does not exists"),))
                    ...
                    if self.check_exists_byclass("nodata"):
                        ...
                        try:
                            with self.conn:
                                self.cur.execute(
                                    'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                    (int(customer), "Medicare CodingDiscontinuation", int(i), str("Overview"), str(val),))
                                # print 'Data saved from exception'
                                ss_name = str(wbpath) + "/" + str(i) + str(
                                    val) +"Medicare-Risk CodingDiscontinuation"+ "Overview" + ".png"  # naming convention is year-lob-drill.png
                            ...
                            if self.check_exists_byclass("nodata"):
                                ...
                                try:
                                    with self.conn:
                                        self.cur.execute(
                                            'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                            (int(customer), "Medicare-CodingDiscontinuation", int(i), str(drill_name), str(val),))
                                        # print 'Data saved from exception'
                                        ...
                                        try:
                                            with self.conn:
                                                self.cur.execute(
                                                    'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                                    (
                                                        int(customer), "Medicare-CodingDiscontinuation", int(i), str(drill_name2),
                                                        str(val),))
                                                # print 'Data saved from exception'
                                                ss_name = str(wbpath) + "/" + str(i) + str(val) + str(
                ...
                if self.check_exists_byclass("nodata"):
                    ...
                    try:
                        with self.conn:
                            self.cur.execute(
                                'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                (int(customer), "Medicare CodingDiscontinuation", int(i), str("Overview"), "Medicare",))
                            # print 'Data saved from exception'
                            ss_name = str(wbpath) + "/" + str(
                                i) + "Medicare" + "CodingDiscontinuation" + ".png"  # naming convention is year-lob-drill.png
                            self.driver.save_screenshot(ss_name)
                    except sqlite3.IntegrityError:
                        pass
                        ...
                        if self.check_exists_byclass("nodata"):
                            ...
                            try:
                                with self.conn:
                                    self.cur.execute(
                                        'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                        (int(customer), "Medicare RiskOverview", int(i), str(drill_name), "Medicare",))
                                    # print 'Data saved from exception'
