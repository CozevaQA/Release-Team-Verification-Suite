...
class TotalCostTrends:
    ...
    def makedir(self, customer):
        path1 = str(customer) + "/" + str(customer) + "-Total Cost Trends"
    ...
    def iterate_filter(self, year, customer):
        ...
        for lob_element in lob_elements:
            ...
            if self.check_exists_byclass("nodata"):
                print("No data found  for lob {}".format( val))
                try:
                    with self.conn:
                        self.cur.execute(
                            'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                            (int(customer), "Total Cost Trends", 2021, str("Overview"),
                             str(val),))
                        # print 'Data saved from exception'
                        ss_name = str(wbpath) + "/"  + str(
                            val) + "Total Cost Trends" + "Overview" + ".png"  # naming convention is year-lob-drill.png
