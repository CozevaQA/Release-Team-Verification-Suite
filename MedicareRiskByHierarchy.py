...
class MedicareRiskByHierarchy:
    ...
    def __init__(self, driver):
        ...
        self.selected_value_year_xpath = '//select[@name="selection"]//following::span[@class="multiselect-selected-text"][1]'
        self.service_year_xpath = '//select[@name="selection"]//following-sibling::div//child::button//child::span'
    ...
    def makedir(self, customer):
        path1 = str(customer) + "/" + str(customer) + "-MedicareRiskByHierarchy"
                    ...
                    try:
                        ...
                        with self.conn:
                            self.cur.execute(
                                ...
                                (int(customer), "Medicare Risk By hierarchy", int(i), str("Does not exists"),
                                 str("Does not exists"),))
                    ...
                    if self.check_exists_byclass("nodata"):
                        ...
                        try:
                            with self.conn:
                                self.cur.execute(
                                    'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                    (int(customer), "Medicare Risk by Hierarchy", int(i), str("Overview"), str(val),))
                                # print 'Data saved from exception'
                                ss_name = str(wbpath) + "/" + str(i) + str(
                                    val) + "Medicare-Risk by Hierarchy" + "Overview" + ".png"  # naming convention is year-lob-drill.png
                            ...
                            if self.check_exists_byclass("nodata"):
                                ...
                                try:
                                    with self.conn:
                                        self.cur.execute(
                                            'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                            (int(customer), "Medicare-Risk by Hierarchy", int(i), str(drill_name),
                                             str(val),))
                                        # print 'Data saved from exception'
                                        ...
                                        try:
                                            with self.conn:
                                                self.cur.execute(
                                                    'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                                    (
                                                        int(customer), "MedicareRisk by Hierarchy", int(i),
                                                        str(drill_name2),
                                                        str(val),))
                                                # print 'Data saved from exception'
                                                ss_name = str(wbpath) + "/" + str(i) + str(val) + str(
                ...
                if self.check_exists_byclass("nodata"):
                    ...
                    try:
                        with self.conn:
                            self.cur.execute(
                                'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                (int(customer), "Medicare RiskByHierarchy", int(i), str("Overview"), "Medicare",))
                            # print 'Data saved from exception'
                            ss_name = str(wbpath) + "/" + str(
                                i) + "Medicare" + "RiskByHierarchy" + ".png"  # naming convention is year-lob-drill.png
                            self.driver.save_screenshot(ss_name)
                    except sqlite3.IntegrityError:
                        pass
                            ...
                            try:
                                with self.conn:
                                    self.cur.execute(
                                        ...
                                        'LOB) VALUES (?,?,?,?,?)',
                                        (int(customer), "Medicare RiskByHierarchy", int(i), str(drill_name),
                                         "Medicare",))
                                    # print 'Data saved from exception'
                                    ...
                                    try:
                                        with self.conn:
                                            self.cur.execute(
                                                'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                                (
                                                    int(customer), "RiskByHierarchy", int(i), str(drill_name2),
                                                    "Medicare",))
                                            # print 'Data saved from exception'
                                            ss_name = str(wbpath) + "/" + str(i) + "Medicare" + str(
                                                drill_name2) + ".png"  # naming convention is year-lob-drill.png
