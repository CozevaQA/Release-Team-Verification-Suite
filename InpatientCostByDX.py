...
class InpatientCostByDX:
    ...
    def makedir(self, customer):
        path1 = str(customer) + "/" + str(customer) + "-Inpatient Cost By DX"
                ...
                try:
                    ...
                    with self.conn:
                        self.cur.execute(
                            ...
                            (int(customer), "Inpatient Cost By DX", int(i), str("Does not exist"),
                             str("Does not  exists"),))
                ...
                if self.check_exists_byclass("nodata"):
                    ...
                    try:
                        with self.conn:
                            self.cur.execute(
                                'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                (int(customer), "Inpatient CostDX", int(i), str("By Service Category"), str(val),))
                            # print 'Data saved from exception'
