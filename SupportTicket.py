...
def SubmitSupportTicket(user,ws):
    global i
    ID = 0
# SupportTicket Submission
    ...
    driver.find_element(By.XPATH, plus_xpath).click()
    time.sleep(2)
    try:
    ...
    try:
        ...
        element2.click()
        time.sleep(2)
        if (len(driver.find_elements(By.XPATH,"//div[@class='modal cozeva-prompt offshore_warning open']"))) > 0:
            driver.find_element(By.XPATH, "//div[@class='modal cozeva-prompt offshore_warning open']").find_element(By.CLASS_NAME, "modal-footer").find_element(By.TAG_NAME, "a").click()
...
#user_dict_1 = {"Customer Support" : "PROV_CA_AshTruong", "Regional Support" : "alena.meza", "Provider" : "mpm_GeTabi"}
...
sf.ajax_preloader_wait(driver)
for k in range(0, len(ticket_detail_list)-1,2):
    try:
