...
def wait_to_load(driver):
    loader=config.get("EDCost-Prod","loader_element")
...
class EDCost:
    ...
    def __init__(self, driver):
        ...
        self.driver = driver
        self.loader_element = config.get("EDCost-Prod","loader_element")
        self.selected_value_year_xpath = config.get("EDCost-Prod","selected_value_year_xpath")
        self.service_year_xpath = config.get("EDCost-Prod","service_year_xpath")
        self.lob_xpath = config.get("EDCost-Prod","lob_xpath")
        self.lob_elements_xpath = config.get("EDCost-Prod","lob_elements_xpath")

        self.apply_filter_xpath = config.get("EDCost-Prod","apply_filter_xpath")

        self.loader_panel_class = config.get("EDCost-Prod","loader_panel_class")
        self.drilldown_elements_xpath = config.get("EDCost-Prod","drilldown_elements_xpath")
        self.select_all_id = config.get("EDCost-Prod","select_all_id")
        self.overview_xpath = config.get("EDCost-Prod","overview_xpath")
        self.workbook_name= "ED Cost"
    ...
    def makedir(self, customer):
        path1 = str(customer) + "/" + str(customer) + "-ED Cost"
                ...
                try:
                    ...
                    with self.conn:
                        self.cur.execute(
                            ...
                            (int(customer), "ED Cost", int(i), str("Does not exist"),
                             str("Does not  exists"),))
                ...
                if self.check_exists_byclass("nodata"):
                    ...
                    try:
                        with self.conn:
                            self.cur.execute(
                                'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                (int(customer), "ED cost", int(i), str("By Dx"), str(val),))
                            # print 'Data saved from exception'
