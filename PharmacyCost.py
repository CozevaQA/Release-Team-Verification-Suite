...
#change  workbook_name variable and first drill down variable
...
class PharmacyCost:
    ...
    def __init__(self, driver):
        ...
        self.driver = driver
        self.loader_element = config.get("PharmacyCost-Prod","loader_element")
        self.selected_value_year_xpath = config.get("PharmacyCost-Prod","selected_value_year_xpath")
        self.service_year_xpath = config.get("PharmacyCost-Prod","service_year_xpath")
        self.lob_xpath = config.get("PharmacyCost-Prod","lob_xpath")
        self.lob_elements_xpath = config.get("PharmacyCost-Prod","lob_elements_xpath")

        self.apply_filter_xpath = config.get("PharmacyCost-Prod","apply_filter_xpath")

        self.loader_panel_class = config.get("PharmacyCost-Prod","loader_panel_class")
        self.drilldown_elements_xpath = config.get("PharmacyCost-Prod","drilldown_elements_xpath")
        self.select_all_id = config.get("PharmacyCost-Prod","select_all_id")
        self.overview_xpath = config.get("PharmacyCost-Prod","overview_xpath")
        self.workbook_name= "Pharmacy Cost"
    ...
    def makedir(self, customer):
        path1 = str(customer) + "/" + str(customer) + "-Pharmacy Cost"
                ...
                try:
                    ...
                    with self.conn:
                        self.cur.execute(
                            ...
                            (int(customer), "Pharmacy Cost", int(i), str("Does not exist"),
                             str("Does not  exists"),))
                ...
                if self.check_exists_byclass("nodata"):
                    ...
                    try:
                        with self.conn:
                            self.cur.execute(
                                'INSERT INTO analytics_nodata_found (Customer,Workbook,Year,DrillDown,LOB) VALUES (?,?,?,?,?)',
                                (int(customer), "Pharmacy cost", int(i), str("By ETC Category"), str(val),))
                            # print 'Data saved from exception'
                            ss_name = str(wbpath) + "/" + str(i) + str(
                                val) + "DX" + ".png"  # naming convention is year-lob-drill.png
